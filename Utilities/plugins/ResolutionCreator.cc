...
ResolutionCreator::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  //  std::vector<TLorentzVector> p4gen;
  std::vector<math::XYZTLorentzVector> p4gen;
  p4gen.clear();

  //  std::vector<TLorentzVector> p4rec;
  std::vector<math::XYZTLorentzVector> p4rec;
  p4rec.clear();


  edm::Handle<TtGenEvent> genEvt; // old
  iEvent.getByLabel ("genEvt",genEvt); // old
  
  edm::Handle<reco::GenParticleCollection> genParticles; // new1
  iEvent.getByLabel("genParticles", genParticles); // new1

  //   if(genEvt->particles().size()<10) return; // old
  //   if(!genParticles.isValid()) continue;
   ...
   if(objectType_ == "electron"){
	 ...
	 if (fabs(part.pdgId())==24 ){
	   int daughters  = part.numberOfDaughters();
	   for(int i = 0;i<daughters;i++){
	     const reco::Candidate *daughter   = part.daughter(i);
	     if(daughter->status() == 1){
	       if( (std::fabs(daughter->pdgId()) == 11) && (ROOT::Math::VectorUtil::DeltaR(daughter->p4(), (*electrons)[e].p4()) < minDR_) ) {
		 cout << "electron pt= " << daughter->pt() << endl;
		 p4gen.push_back(daughter->p4());
		 p4rec.push_back((*electrons)[e].p4());
	       }
	     }else {
	       int daughters_sec = daughter->numberOfDaughters();
	       for(int ij = 0; ij<daughters_sec; ij++){
		 const reco::Candidate *daughter_sec   = daughter->daughter(ij);
		 if(daughter_sec->status() == 1){
		   if( (std::fabs(daughter_sec->pdgId()) == 11) && (ROOT::Math::VectorUtil::DeltaR(daughter_sec->p4(), (*electrons)[e].p4()) < minDR_) ){
		     p4gen.push_back(daughter_sec->p4());
		     p4rec.push_back((*electrons)[e].p4());
	 ...
	 if (fabs(part.pdgId())==24 ){
	   ...
	   for(int i = 0;i<daughters;i++){
	     const reco::Candidate *daughter   = part.daughter(i);
	     //	     if(daughter->status() == 1){
	       if( (std::abs(daughter->pdgId()) == 13) && (ROOT::Math::VectorUtil::DeltaR(daughter->p4(), (*muons)[m].p4()) < minDR_) ) {                   
		 //		 cout << "muon pt= " << daughter->pt() << endl;
                 p4gen.push_back(daughter->p4());
                 p4rec.push_back((*muons)[m].p4());

	       } // final condition loop
	       //	     } // daughter status loop
	   } // daugher for loop
	 } // partile mother finding loop
       } // gen particle loop
     } // rec-muon 
   } // 
   ...
   */
	 ...
	 for(size_t i = 0; i < genParticles->size(); ++ i) {
	   const reco::GenParticle & part = (*genParticles)[i];
           if( (std::abs(part.pdgId()) < 5) && (ROOT::Math::VectorUtil::DeltaR(part.p4(), (*jets)[j].p4())< minDR_) ){
	     //	     cout << "light jet's pt= " << part.pt() << endl;
